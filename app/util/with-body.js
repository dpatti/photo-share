...
// type ExtendedContext = Context & {request: {body: JSON}};

module.exports = <T>(refine: (JSON => boolean), handler: Handler<T>): Middleware =>
  async (context: Context, next: Continuation) => {
    await bodyParser(context, () => {
      // TODO: See note above ExtendedContext.
      // eslint-disable-next-line flowtype/no-weak-types
      const body: JSON = (context: any).request.body;
      if (refine(body)) {
